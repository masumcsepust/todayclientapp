...
import { MemberAddEdit } from 'src/app/shared/models/admin/memberAddEditDto';
...
export class AddEditMemberComponent implements OnInit {
  ...
  initializeForm(member: MemberAddEdit | undefined) {
    if(member) {
      this.memberForm = this.formBuilder.group({
        ...
        lastName: [member.firstName, Validators.required],
  ...
  getRoles() {
    this.adminService.getApplicationRoles().subscribe( roles => {
  ...
  passwordOnChange() {
    if(this.addNew ==false) {
      if(this.memberForm.get('password')?.value) {
        this.memberForm.controls["password"].setValidators([Validators.required, Validators.minLength(5), Validators.maxLength(15)])
      } else {
  ...
  submit() {
    ...
    if(this.memberForm.valid) {
      this.adminService.addEditMember(this.memberForm.value).subscribe( (res: any) => {
